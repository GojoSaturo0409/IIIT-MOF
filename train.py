#!/usr/bin/env python3
"""
MAE pretraining / fine-tuning on 3D voxelized MOFs with DDP + test predictions to CSV.

Key features:
- Increased NCCL timeout (3600s) to prevent watchdog timeouts
- Modern torch.amp API (no deprecated torch.cuda.amp)
- Safer DDP process cleanup with try-finally
- **NEW: Test predictions saved to CSV file**
- Per-epoch checkpoint persistence
"""
import os
import argparse
import math
...
def _safe_barrier(distributed, device):
    """Safely execute a barrier with error handling."""
    if not distributed:
        return
    try:
        if device.type == "cuda":
...
def evaluate_and_save_csv(model, loader, device, args, reg_head=None, distributed=False, output_csv=None):
    ...
    Args:
        ...
        args: Arguments dict
        reg_head: Optional regression head
        distributed: Whether using DDP
        output_csv: Path to save CSV file (if None, no CSV saved)
    ...
    # For CSV output (rank 0 only collects these)
    all_filenames = []
    all_predictions = []
    all_targets = []
    all_valid_flags = []
    ...
    with torch.no_grad():
        for vox_batch, target_batch, names in loader:
            ...
            if reg_eval is not None:
                ...
                # Denormalize predictions for CSV
                if args.normalize_target and hasattr(args, '_target_mean'):
...
def evaluate(model, loader, device, args, reg_head=None, distributed=False):
    """Original evaluate function for validation (no CSV)."""
...
def load_checkpoint_if_requested(args, model, opt, sched, scaler, reg_head=None, device=None, distributed=False, is_main=True):
    """Load checkpoint and return (start_epoch, best_loss, patience_cnt, bad_batches)."""
    start_epoch = 1
...
# -------------------------
def train(args):
    """Main training function with improved DDP robustness."""
    # ===== DDP SETUP =====
    distributed = False
    device = None
    is_main = True
    ...
    try:
        ...
        # ===== TEST EVALUATION WITH CSV OUTPUT =====
